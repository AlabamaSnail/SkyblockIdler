...
var skill_speed = (100 * pow(0.8, StatHolder.fishing)) * pow(2.0, (StatHolder.fishingRebirthUpgrade * 1.0))
#L: 1 * (L**2 + 1)


func _ready() -> void:
	self.value = StatHolder.fishingProgress
	skill_speed = (100 * pow(0.8, StatHolder.fishing)) * pow(2.0, (StatHolder.fishingRebirthUpgrade * 1.0))
func _process(delta):
