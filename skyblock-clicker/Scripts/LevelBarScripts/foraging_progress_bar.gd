...
var skill_speed = (100 * pow(0.8, StatHolder.foraging)) * pow(2.0, (StatHolder.foragingRebirthUpgrade * 1.0))

#L: 1 * (L**2 + 1)

func _ready() -> void:
	self.value = StatHolder.foragingProgress
	skill_speed = (100 * pow(0.8, StatHolder.foraging)) * pow(2.0, (StatHolder.foragingRebirthUpgrade * 1.0))
func _process(delta):
