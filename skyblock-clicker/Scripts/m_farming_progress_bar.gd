...
func _process(delta):
	...
	if StatHolder.MoneyfarmingToggle:
		...
		StatHolder.MoneyfarmingProgress += (StatHolder.farmingMoneySpeed / StatHolder.MoneyfarmingStacks ) * delta
		...
		if StatHolder.MoneyfarmingProgress > 100:
			...
			StatHolder.money.plusEquals(StatHolder.farmingMoneyMulti)  # Add to money based on stacks
...
func _on_mf_stacks_up_pressed() -> void:
	if not StatHolder.MoneyfarmingToggle:
		...
		StatHolder.farmingMoneySpeed -= 100  # Permanently subtract 100 from skill_speed
...
func _on_mf_stacks_down_pressed() -> void:
	...
	if StatHolder.MoneyfarmingStacks  > 1:
		...
		StatHolder.farmingMoneySpeed += 100  # Add 100 back to skill_speed
		_update_stacks_label()  # Update the label to reflect the new stacks value
